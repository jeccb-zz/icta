...
const replaceIdea = (ideas, newIdea, index) =>
  (ideas.slice(0, index).concat([newIdea]).concat(ideas.slice(index + 1)));
...
const ideas = (state = [], action) => {
  switch (action.type) {
    ...
    case EDIT_IDEA_RECEIVED: {
      ...
      return sortIdeas(replaceIdea(state, editedIdea, editedIndex));
    }
    case CHANGE_MY_VOTE: {
    ...
    case VOTE_SUCCESS: {
      ...
      return sortIdeas(replaceIdea(state, newIdea, index));
    }
    case VOTE_REMOVE_SUCCESS: {
      const indexToRemove = state.map(i => i.id).indexOf(action.idea.id);
      const newIdeaRemoved = {
        ...state[indexToRemove],
        up: action.idea.up,
        down: action.idea.down,
      };

      return sortIdeas(replaceIdea(state, newIdeaRemoved, indexToRemove));
    }
    case DENY_IDEA_SUCCESS:
    case DELETE_IDEA_SUCCESS:
      return state.filter(i => Number(i.id) !== Number(action.ideaId));
    default:
      return state;
  }
};
